...
switch ($method) {
    case 'GET':
        ...
        if (end($urlParts) === 'homes') {
            ...
            $stmt = $db->query("SELECT a.* FROM `home_about` h INNER JOIN about a ON a.aboutId = h.aboutId WHERE h.status = 'A' order by h.seq asc");
            ...
            $stmt = $db->query("SELECT p.* FROM `home_projects` h INNER JOIN projects p ON p.projectId = h.projectId WHERE h.status = 'A' order by h.seq asc");
            $homeProject = $stmt->fetchAll(PDO::FETCH_ASSOC);

            $stmt = $db->query("SELECT * FROM publications WHERE status = 'A' ORDER BY seq DESC LIMIT 8 ");
